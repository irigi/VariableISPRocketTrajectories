...
We restrict attention to the symmetric boost--coast--brake family obtained by scaling the unconstrained PMP profile of Sec.~\ref{sec:free_space_solution} by a constant throttle factor and inserting a coast interval at mid-time. Enforcing the distance, propellant, and exhaust-speed constraints selects a unique member of this family for the prescribed boundary conditions.\footnote{This scaled-linear profile is generally not the PMP extremal of the state-constrained problem; the true constrained extremal includes a boundary arc along which \(v_e\equiv v_{e,\max}\), where the acceleration is no longer linear in time. The solution below is the optimum within the assumed family---useful as a closed-form approximation for engineering estimates and as a starting point for refined indirect or direct solvers.}
...
\end{equation}
The function \(F(\xi)\) is strictly decreasing on \((0,\tfrac12)\),\footnote{\(f'(\xi)=3(2\xi-1)^2>0\) makes the second term of \eqref{eq:root_equation_new} increase while the first decreases. The boundary values \(F(0^+)=\tfrac12 v_{e,\max}>0\) and \(F(\tfrac12)<0\) then guarantee a unique root.} so a unique root \(\xi_\star\in(0,\tfrac12)\) exists for any finite positive exhaust-speed ceiling.

\subsection{Final Solution and Limiting Cases}
Within the symmetric boost--coast--brake family, the capped transfer is found by solving \(F(\xi)=0\) for \(\xi_\star\in(0,\tfrac12)\) with any robust root-finder, then evaluating \eqref{eq:T_of_xi_new}. Because the uncapped profile requires an infinite exhaust speed at mid-time, any finite positive \(v_{e,\max}\) inserts a coast interval; the unlimited-engine benchmark \eqref{eq:Tmin} is recovered only as \(v_{e,\max}\to\infty\), where \(\xi_\star\to\tfrac12\) and the coast duration vanishes.
