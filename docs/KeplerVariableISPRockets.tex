...
\title{Fast Heliocentric Transfers With a Constant-Power, Variable-$I_{\!sp}$ Engine}
...
In this paper, we develop the analytical and numerical machinery for studying the time-optimal transfer problem in this high-power regime. We first derive an analytical solution for a 1D, gravity-free "brachistochrone" to build intuition. We then formulate the full planar Sun-field problem, derive two closed-form first integrals to simplify the dynamics, and present a robust numerical strategy to solve the resulting boundary-value problem. Our idealized examples for a gigawatt-scale engine demonstrate Earth--Jupiter-class transfers in a few months, frequently exploiting a solar Oberth maneuver.
...
\]
diverges when \(a(t)\to0\) at mid-time. A realistic thruster, however, is limited to a maximum effective exhaust speed $v_{e,\max}$. This path constraint forces the optimal trajectory to switch the engine \emph{off} once the required $v_e$ reaches the ceiling, producing the well-known \emph{boost-coast-brake} structure \cite{Fogel2020, pontani2021optimal}.
...
This reduction applies to the ideal variable-\(I_{\!sp}\) model without an imposed finite exhaust-speed ceiling. If a finite ceiling \(v_{e,\max}\) is imposed in the Sun-field problem, the similarity space gains one additional dimensionless parameter,
\begin{equation}
    \nu_{\max}
    \equiv
...
\end{equation}
and the reduced family is parameterized by
\((\rho,\kappa,\Theta,\nu_{\max})\) rather than only by
\((\rho,\kappa,\Theta)\).
...
Figures~\ref{fig:oberth_figure} and~\ref{fig:oberth_figure2} show example trajectories. A key feature of these high-performance paths is the inclusion of an Oberth maneuver. To gain orbital energy efficiently, the spacecraft first thrusts to lower its periapsis, making a close pass of the Sun. It then applies its main acceleration burn at maximum velocity near the Sun, which is far more effective at increasing orbital energy than thrusting in a higher orbit \cite{Oberth1923}. This is visible in the plots as an initial dip towards the Sun and a corresponding peak in the acceleration magnitude. These Oberth-type solutions should be interpreted within the idealized dynamical model used here. No explicit constraints are imposed on minimum perihelion distance, solar heating, radiator performance near the Sun, pointing restrictions, or power-system survivability. Adding such constraints would generally increase the transfer times or remove some of the most aggressive solar-pass solutions.
