...
\bigskip
\section{Finite-Exhaust-Speed Extension in Free Space}
\label{sec:capped_ve}
The uncapped free-space solution of Sec.~\ref{sec:free_space_solution} requires
\begin{equation}
    v_e(t)=\frac{2P}{m(t)|a(t)|},
\end{equation}
and therefore demands arbitrarily large exhaust speed when the affine acceleration passes through zero. This section states the resulting finite-ceiling structure needed for interpreting the ideal model. The full PMP derivation, switching analysis, closed-form arc maps, and endpoint formulas are given in Appendix~\ref{sec:appendix_capped_derivation}.

We impose
\begin{equation}
    v_e\le V,\qquad V\equiv v_{e,\max},
\end{equation}
under a two-mode engine model: when on, the engine supplies the fixed useful jet power $P$; when off, the spacecraft coasts. Define
\begin{equation}
    F\equiv\frac{2P}{V},
    \qquad
    \sigma\equiv\frac{2P}{V^2}.
\end{equation}
At full power the ceiling is equivalent to a minimum nonzero acceleration magnitude, so the admissible signed controls are
\begin{equation}
    \mathcal U(m)
    =\{0\}\cup\left(-\infty,-\frac{F}{m}\right]
              \cup\left[\frac{F}{m},\infty\right).
    \label{eq:capped_summary_control_set}
\end{equation}
Thus accelerations with magnitude between zero and $F/m$ cannot be produced at fixed power; the engine must instead be switched off.

\subsection{Pointwise Selector}
For every finite normal maximizer, the product
\begin{equation}
    C\equiv\lambda_m m^2>0
\end{equation}
is constant. The scalar primer acceleration is therefore affine in time,
\begin{equation}
    p(t)\equiv\frac{P\lambda_v(t)}{C}=p_0-\omega t,
    \qquad \omega>0.
\end{equation}
Writing $q(m)=F/m$, pointwise maximization of the Hamiltonian gives
\begin{equation}
    a^*(t)=
    \begin{cases}
      p(t), & |p(t)|\ge q,\\[1ex]
      ...
      0, & |p(t)|\le q/2.
    \end{cases}
    \label{eq:capped_summary_selector}
\end{equation}
The three corresponding arc types are:
\begin{itemize}
    \item an interior powered arc $U_\pm$, on which $a=p$ and $v_e<V$;
    \item a maximum-exhaust-speed boundary arc $B_\pm$, on which
    $a=\pm F/m$, $v_e=V$, and $\dot m=-\sigma$;
    \item a coast $C$, on which $a=0$ and $m$ is constant.
\end{itemize}

\subsection{Main Structural Result}
For a rest-to-rest transfer from $x=0$ to $x=L>0$, every regular normal extremal with finitely many switches is an endpoint truncation of
\begin{equation}
    [U_+]\;B_+\;C\;B_-\;[U_-\;[B_-]].
    \label{eq:capped_summary_topology}
\end{equation}
Brackets denote optional arcs. Hence the trajectory can begin with an interior acceleration arc, but it must pass through a positive maximum-exhaust-speed boundary arc before entering a central coast. Braking begins on a negative boundary arc and may subsequently enter an interior braking arc and, for some parameter regimes, a second boundary arc. The final state may occur on any displayed braking arc.

For every finite $V$, the central coast has positive duration: the affine primer crosses zero while the powered/coast switching threshold $F/(2m)$ remains positive. Because mass is lost during the powered arcs, the accelerating and braking branches are generally unequal; the finite-ceiling solution is not obtained by uniformly scaling the symmetric uncapped profile.

Convexification of the on--off control set formally admits a coast/boundary sliding extremal on the braking side. It is not time minimizing in the gravity-free problem. An exchange argument shows that, for fixed total boundary-burn time and coast time, moving every coast interval ahead of every braking burn preserves terminal mass and velocity while increasing distance. The excess distance can then be removed by shortening the coast, producing the same endpoint in less time. Thus the relaxed sliding arc and finite coast/boundary chattering are excluded from a true gravity-free minimum-time solution. The proof is given in Appendix~\ref{sec:appendix_capped_derivation}.

\subsection{Endpoint Reduction}
The remaining regular candidates can be computed without integrating a general boundary-value problem. Let $M$ be the mass on the central coast, let $\omega$ be the primer slope, and write the terminal primer as $p(t_f)=-u_f$ with $u_f>0$. Then
\begin{equation}
    t_f=\frac{p_0+u_f}{\omega}.
\end{equation}
For prescribed $(P,V,L,m_i,m_\mathrm{dry})$, the switching law determines $p_0$ from $(M,\omega)$, and forward propagation of the braking branch to $m=m_\mathrm{dry}$ determines $u_f$. The two unknowns $(M,\omega)$ are fixed by
\begin{align}
    0&=\frac{1}{\omega}
       \int_{-u_f}^{p_0}a^*(p,m(p))\,dp,
       \label{eq:capped_summary_velocity}\\
    L&=\frac{1}{\omega^2}
       \int_{-u_f}^{p_0}p\,a^*(p,m(p))\,dp.
       \label{eq:capped_summary_distance}
\end{align}
All arc propagations and switching masses are elementary; their closed forms and the topology-dependent admissibility tests are collected in Appendix~\ref{sec:appendix_capped_derivation}.

As $V\to\infty$, the boundary arcs and central coast shrink to zero duration and the uncapped affine solution is recovered. The orbital calculations in Secs.~\ref{sec:SunModel}--\ref{sec:similarity_reduction} and the numerical examples below use that ideal \emph{uncapped} model. They should therefore be interpreted as a benchmark for the continuously variable-$I_{\!sp}$ limit, not as trajectories satisfying a specified finite exhaust-speed ceiling. For any representative numerical trajectory, the implied exhaust-speed history is recovered a posteriori from
\begin{equation}
    v_e(t)=\frac{2P}{m(t)\|\mathbf a_T(t)\|}.
    \label{eq:ve_diagnostic_uncapped}
\end{equation}
A finite-ceiling heliocentric study would replace the unconstrained thrust law by the vector analogue of Eq.~\eqref{eq:capped_summary_selector} and is left for future work.
...
\section{Conclusion}
We combined analytic insight (two closed-form first integrals, an uncapped free-space benchmark, and a finite-exhaust-speed structural extension) with a continuation strategy for constructing time-optimal transfer candidates for a constant-power, variable-\(I_{\!sp}\) engine. The uncapped numerical method generates PMP-extremal candidate trajectories, including those featuring low-perihelion Oberth-like maneuvers, for transfers out to the giant planets with a gigawatt-class useful jet-power engine. Recent developments in variable-$I_{\!sp}$ propulsion and adaptive low-thrust guidance algorithms \cite{PinoHowell2020,DiPasquale2024} suggest that further improvements in mission performance are possible. Future work will extend this framework to three-dimensional rendezvous problems. A rigorous demonstration of local or global time optimality for the Sun-field examples remains future work; it would require either second-order sufficiency analysis of the extremals, such as conjugate-point checks, or a broader global comparison across competing transfer branches.
...
\appendices
\section{Complete Derivation for the Finite Exhaust-Speed Free-Space Problem}
\label{sec:appendix_capped_derivation}
The main text states only the selector, regular topology, sliding exclusion, and endpoint reduction. This appendix gives the complete derivation and the topology-dependent closed forms.
