...
\begin{equation}
  ...
  \label{eq:mass_flow}
\end{equation}
In vector problems, \(a\) in \eqref{eq:mass_flow} should be interpreted as the thrust-acceleration magnitude, \(a=\|\mathbf a_T\|\). In the one-dimensional free-space derivation below, we use \(a(t)\) as a signed scalar acceleration; because the mass-flow law depends on \(a(t)^2\), this sign convention is immaterial for the propellant balance, while the physical thrust magnitude is \(m|a(t)|\).
...
\[
    J=\int_0^{t_f} 1\,dt,
\]
so the running cost is \(L=1\). The sign convention \(\mathcal H=\boldsymbol\lambda^\top \mathbf f-L\) used here is Pontryagin's original ``normal-branch'' convention \cite{Pontryagin1962}, in which the optimal control maximizes \(\mathcal H\) pointwise; many modern aerospace optimal-control texts instead write \(\mathcal H=\boldsymbol\lambda^\top \mathbf f+L\) and minimize, which differs from \eqref{eq:hamiltonian_general} by the sign of the costates. For an autonomous problem with free final time, transversality and conservation of \(\mathcal H\) along the optimal trajectory then imply \(\mathcal H\equiv 0\).
...
\end{equation}
For this stationary point to be a maximum of the Hamiltonian with respect to the acceleration control, the normal solution branch requires
\begin{equation}
    \lambda_m>0,
    \qquad
    C_m=\lambda_m m^2>0.
\end{equation}
With this sign choice, the Hamiltonian is concave in the scalar acceleration \(a\).
...
\label{sec:capped_ve}
The linear-in-time acceleration profile from Sec.~\ref{sec:free_space_solution} is feasible only if the engine can raise its exhaust speed indefinitely, because the required exhaust speed,
\[
    v_e(t)=\frac{2P}{m(t)|a(t)|},
\]
diverges when \(a(t)\to0\) at mid-time. A realistic thruster, however, is limited to a maximum effective exhaust speed \(v_{e,\max}\). In this section we assume that the thruster has two admissible modes: a powered mode with fixed useful jet power \(P\) and \(v_e\le v_{e,\max}\), and a coast mode with \(P=0\) and \(a=0\). Under this model, the finite exhaust-speed ceiling inserts a coast interval, producing a boost--coast--brake structure \cite{Fogel2020, pontani2021optimal}.
...
We restrict attention here to a specific candidate family, namely the symmetric boost--coast--brake structure obtained by scaling the unconstrained PMP profile of Sec.~\ref{sec:free_space_solution} by a constant throttle factor and inserting a coast interval at mid-time. The Pontryagin stationarity conditions are then preserved on each powered arc, so the trajectory is admissible. Enforcing the distance, propellant, and exhaust-speed constraints selects a unique member of this family for the prescribed boundary conditions.

We emphasize that this scaled-linear profile is generally \emph{not} the PMP extremal of the state-constrained problem. The true constrained extremal will typically include a boundary arc along which \(v_e\equiv v_{e,\max}\), and on which the acceleration profile is no longer linear in time. The closed-form solution below is therefore the optimum within the assumed family rather than the global optimum of the capped problem, and it provides a tractable closed-form approximation useful for engineering estimates and as a starting point for refined indirect or direct solvers.

\paragraph{Distance Closure}
The total distance traveled, \(L\), is the sum of the first burn, the coast, and the final braking burn. It is useful to distinguish the position polynomial
\[
    g(s)\equiv 3s^2-2s^3
\]
from the inverse-mass polynomial \(f(s)=4s^3-6s^2+3s\) defined in \eqref{eq:fdef}. During the first powered arc, the scaled displacement is
\[
    x_\mathrm{burn}=kL\,g(\xi).
\]
The velocity at the beginning of the coast is
\[
    v_c
    =
    k\,\frac{6L}{t_f}\,\xi(1-\xi),
\]
and the coast duration is \((1-2\xi)t_f\), so the coast displacement is
\[
    x_\mathrm{coast}
    =
    kL\,6\xi(1-\xi)(1-2\xi).
\]
By symmetry, the final braking displacement is another \(kL\,g(\xi)\). Therefore
\begin{align}
    L
    &=
    kL\left[
      2g(\xi)+6\xi(1-\xi)(1-2\xi)
    \right] \nonumber\\
    &=
    kL\left[
      6\xi-12\xi^2+8\xi^3
    ...
    2kL\,f(\xi).
\end{align}
Thus the distance closure is
\begin{equation}
    k = \frac{1}{2f(\xi)}.
    \label{eq:k_capped}
\end{equation}
This expression includes the coast distance; the equality follows because the coast contribution combines with the two powered-arc displacements to give \(2f(\xi)\).
...
\end{equation}
Solving this for the total time \(t_f\) and substituting \eqref{eq:k_capped} yields the flight time as a function of the switch fraction:
\begin{equation}
t_f(\xi)=
\Biggl[
...
The final constraint is that the exhaust speed must not exceed its ceiling,
\(v_e(t)\le v_{e,\max}\). On each powered arc,
\[
    v_e(t)=\frac{2P}{m(t)|a(t)|}.
\]
The critical point occurs at the switch-off time \(t=t_c\), where the acceleration magnitude is smallest on the powered arc. We impose the ceiling exactly at this point:
\begin{equation}
    v_e(t_c)
    =
    \frac{2P}{m_c |a(t_c)|}
    ...
    v_{e,\max}.
\end{equation}
The mass at this point is
\begin{equation}
    m_c
    =
    \frac{2m_i m_\mathrm{dry}}{m_i+m_\mathrm{dry}},
    \label{eq:mc_harmonic}
\end{equation}
because the symmetric first burn consumes one half of the available inverse-mass budget,
\[
    \frac{1}{m_c}-\frac{1}{m_i}
    =
    \frac12
    \left(
    ...
    \right).
\]
The optimal switch fraction \(\xi_\star\) is then the unique root in \((0,\tfrac12)\) of
\begin{equation}
F(\xi)\;\equiv\;
...
\[
    ...
    \text{for }0<\xi<\tfrac12,
\]
the function \(F(\xi)\) is strictly decreasing on \(0<\xi<\tfrac12\).
Moreover,
\[
    F(0^+)=\frac12 v_{e,\max}>0,
    \qquad
    F(\tfrac12)<0.
\]
Therefore, for any finite positive exhaust-speed ceiling, a unique root
\(\xi_\star\in(0,\tfrac12)\) exists.

\subsection{Final Solution and Limiting Cases}
Within the symmetric boost--coast--brake family considered here, the capped transfer is determined by solving the scalar equation \(F(\xi)=0\) for the root \(\xi_\star \in (0,1/2)\) using any robust one-dimensional method. This value is then used in \eqref{eq:T_of_xi_new} to compute the corresponding transfer time.

\paragraph*{Activity of the ceiling}
For the ideal unlimited-\(I_{\!sp}\) solution, the required exhaust speed diverges at mid-time. Consequently, any finite positive value of \(v_{e,\max}\) makes the ceiling active and inserts a coast interval. The unlimited-engine benchmark \eqref{eq:Tmin} is recovered only in the limiting case \(v_{e,\max}\to\infty\), for which \(\xi_\star\to\tfrac12\) and the coast duration tends to zero.
...
\item \textbf{Small ceiling.} As \(v_{e,\max}\) becomes small, the root of \(F(\xi)=0\) moves toward \(\xi\to0^+\). The powered portions then occupy a vanishing fraction of the total transfer time, while the coast interval dominates. In this limiting sense the structure approaches two short powered arcs separated by a long ballistic cruise.
\end{itemize}
...
\begin{equation}
    C=\lambda_m m^2>0,
\end{equation}
so that \(k_T=P/C>0\). With this sign choice, the Hamiltonian is concave in the thrust-acceleration components \((a_r,a_\theta)\), and the stationary control indeed maximizes \(\mathcal H\) with respect to the acceleration control.
...
This reduction applies to the ideal variable-\(I_{\!sp}\) model without an imposed finite exhaust-speed ceiling. If a finite ceiling \(v_{e,\max}\) is imposed in the Sun-field problem, the similarity space gains additional dimensionless parameters. One is the normalized exhaust-speed ceiling
\begin{equation}
    \nu_{\max}
    \equiv
    \frac{v_{e,\max}}{\sqrt{\mu/r_0}},
    \label{eq:nu_max_similarity}
\end{equation}
and another is the mass ratio
\begin{equation}
    \eta
    \equiv
    \frac{m_i}{m_\mathrm{dry}}.
    \label{eq:eta_similarity}
\end{equation}
The need for \(\eta\) follows because the capped constraint contains the dimensional mass through
\[
    v_e=\frac{2P}{m\|\mathbf a_T\|}.
\]
Using the normalized variables below,
\begin{equation}
    \frac{v_e}{\sqrt{\mu/r_0}}
    =
    \frac{\kappa}
         {m\Delta\,\|\bar{\mathbf a}_T\|},
    \label{eq:ve_scaled_similarity}
\end{equation}
and
\begin{equation}
    m\Delta
    =
    \frac{\eta-1}{1+(\eta-1)q}.
    \label{eq:mDelta_eta_similarity}
\end{equation}
Thus the capped Sun-field family is parameterized by
\[
    (\rho,\kappa,\Theta,\nu_{\max},\eta),
\]
whereas the uncapped model depends only on
\[
    (\rho,\kappa,\Theta).
\]
