...
\begin{equation}
  ...
  \label{eq:mass_flow}
\end{equation}
In vector problems, \(a\) in \eqref{eq:mass_flow} should be interpreted as the thrust-acceleration magnitude, \(a=\|\mathbf a_T\|\). In the one-dimensional free-space derivation below, we use \(a(t)\) as a signed scalar acceleration; the mass-flow law then depends on \(a(t)^2\), while the physical thrust magnitude is \(m|a(t)|\).
...
\[
    J=\int_0^{t_f} 1\,dt,
\]
so the running cost is \(L=1\). With this convention, the optimal control maximizes \(\mathcal H\) pointwise, and for a normal free-final-time solution the Hamiltonian is constant and satisfies \(\mathcal H\equiv0\).
...
Since the Pontryagin stationarity conditions are preserved on the powered arcs, the scaled linear-in-time profile with an inserted coast is an admissible PMP candidate within the assumed symmetric boost--coast--brake family. Enforcing the distance, propellant, and exhaust-speed constraints selects the member of this family consistent with the prescribed boundary conditions.
...
\[
    ...
    \right).
\]
Substituting the expressions for \(a(t_c)\), \(k\), and \(t_f(\xi)\) into the ceiling condition gives a single nonlinear equation for the optimal switch fraction \(\xi_\star\):
\begin{align}
...
\section{Results}
The numerical procedure was applied to compute PMP-extremal transfer candidates from a 1\,AU circular orbit (representing Earth) to various target circular orbits. The targets are defined by their final radius and true anomaly relative to the starting position. The propulsion model used \(P=\SI{1}{GW}\) of useful jet power, with an initial vehicle mass of \SI{3000}{t} and a dry mass of \SI{1000}{t}.
...
%\begin{table*}[!htbp]
%\centering
%\caption{Transfer time, in days, of the converged PMP-extremal candidate to a circular orbit of a given %radius and final true anomaly.}
...
\caption{Transfer time, in days, of the converged PMP-extremal candidate from a circular orbit of 1 AU and final true anomaly (at rocket arrival).}
...
\footnotesize
\renewcommand{\arraystretch}{0.95}
\caption{Transfer time, in days, of the converged PMP-extremal candidate from a circular orbit of 1 AU and initial true anomaly (at rocket departure).}
...
\begin{table*}[!htbp]
\centering
\caption{Transfer time, in days, of the converged PMP-extremal candidate to a circular orbit of 1 AU and final true anomaly (at rocket arrival).}
...
\begin{table*}[!htbp]
\centering
\caption{Transfer time, in days, of the converged PMP-extremal candidate to a circular orbit of 1 AU and initial true anomaly (at rocket departure).}
